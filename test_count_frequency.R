...
test_that("Test count_frequency with a minimum token length", {
  cmd <- "count_frequency"
  args <- "samples/events2013.dat freqs5_events2013.dat 5"
  result <- system2(cmd, args)
  expect_equal(0, result, info="Unexpected return code")
  expect_true(file.exists("freqs5_events2013.dat"),
              info="Could not find freqs5_events2013.dat")
  expected <- read.table("testoracle/freqs5_events2013.dat",
                         stringsAsFactors=FALSE,
                         colClasses=("numeric"))
  actual <- read.table("freqs5_events2013.dat",
                       stringsAsFactors=FALSE,
                       colClasses=("numeric"))
  expect_equivalent(dim(expected),
                    dim(actual),
                    info="Dimensions of freqs5_events2013.dat not equal to testoracle")
  expect_equal(expected,
               actual,
               tolerance=test.tolerance,
               info="freqs5_events2013.dat not equal to testoracle")
})

test_that("Test count_frequency with a missing output file name", {
  cmd <- "count_frequency"
  args <- "samples/events2013.dat"
  result <- system2(cmd, args)
